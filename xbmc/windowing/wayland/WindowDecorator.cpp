...
void CWindowDecorator::UpdateSeatCursor(Seat& seat)
{
  ...
  {
    CSingleLock lock(m_mutex);
    auto resizeEdge = ResizeEdgeForPosition(seat.currentSurface, SurfaceGeometry(seat.currentSurface, m_mainSurfaceSize).ToSize(), seat.pointerPosition);
...
void CWindowDecorator::UpdateButtonHoverState()
{
  ...
  for (auto& button : m_buttons)
  {
    ...
    button.hovered = std::any_of(pointerPositions.cbegin(), pointerPositions.cend(), [&](CPoint point) { return button.position.PtInRect(point); });
  ...
  switch (button)
  {
    case BTN_LEFT:
    {
      ...
      if (resizeEdge == wayland::shell_surface_resize::none)
      {
        ...
        {
          if (button.position.PtInRect(position))
          {
            button.onClick();
            return;
          }
    ...
    case BTN_RIGHT:
      ...
      {
        m_handler.OnWindowShowContextMenu(seat, serial, position - CPoint{BORDER_WIDTH, BORDER_WIDTH + TOP_BAR_HEIGHT});
      }
      break;
  }
}
...
void CWindowDecorator::CommitAllBuffers()
{
  ...
  for (auto& surface : m_surfaces)
  {
    ...
    CLog::LogF(LOGDEBUG, "-------------------- pending buffer size: %u", m_pendingBuffers.size());
